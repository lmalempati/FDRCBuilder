...
//        updateCompletionPayload(response, "payloads/Credit-Visa-AuthComple-swipe-200019960021.json");
    ...
    public void creditDSAuthKeyed200070910010() {
        Response response = new Client().processRequest(JsonBuilder.getRequestFromJson(PATH + "DS-Auth-Keyed-200070910010.json"));
        assertEquals("002", response.respCode);
    }

    @Test // AuthVoid or ReversalRequest
    public void creditAuthVoidKeyed200070840010() {
        Response response = new Client().processRequest(JsonBuilder.getRequestFromJson(PATH + "credit-AuthVoid-200070840010.json"));
        assertEquals("000", response.respCode);
        if (JsonBuilder.updateCompletionPayload(response, PATH +"credit-AuthVoid-200070840011.json")) {
    ...
    public void creditAuthVoidKeyed200070840011() {
        Response response;
        response = new Client().processRequest(JsonBuilder.getRequestFromJson(PATH + "credit-AuthVoid-200070840011.json"));
        assertEquals("000", response.respCode);
    }

    public void creditJCBAuthKeyed100070950011() {
        Response response = new Client().processRequest(JsonBuilder.getRequestFromJson(PATH + "JCB-Compl-Keyed-100070950011.json"));
        assertEquals("000", response.respCode);
    }

    @Test // Refund
    public void refundTest() {
        Response response = new Client().processRequest(JsonBuilder.getRequestFromJson(PATH + "Credit-MC-Refund-Keyedin.json"));
        assertEquals("000", response.respCode);
    }

    @Test // Refund Void
    public void refund200081230010() {
        Response response = new Client().processRequest(JsonBuilder.getRequestFromJson(PATH + "Credit-MC-Refund-Keyed-200081230010.json"));
        assertEquals("000", response.respCode);
        if (JsonBuilder.updateCompletionPayload(response, PATH + "Credit-MC-RefundVoid-Keyed-200081230011.json")) {
            refundVoid200081230011();
        }
    }

    public void refundVoid200081230011() {
        Response response = new Client().processRequest(JsonBuilder.getRequestFromJson(PATH + "Credit-MC-RefundVoid-Keyed-200081230011.json"));
        assertEquals("000", response.respCode);
...
//                    "credit-AuthVoid-200070840010.json;000", "credit-AuthVoid-200070840011.json;000", // Auth & Compl..
    ...
    public void runParameterizedTests(String fileName) {
        ...
        Response response = new Client().processRequest(JsonBuilder.getRequestFromJson(PATH + fileSaleAuth));
...
//        JsonBuilder.updateCompletionPayload(response, "" + values[3]);
